...
export default function Favorites() {
  ...
  const removeFavorite = (clubId) => {
    try {
      const updatedFavorites = favorites.filter((fav) => fav.id !== clubId);
  ...
  const filteredFavorites = useMemo(() => {
    let filtered = favorites.filter((club) => {
      if (filterBy === 'all') return true;
      return club.country === filterBy;
    });

    return filtered;
  }, [favorites, filterBy]);

  const countries = [...new Set(favorites.map((club) => club.country).filter(Boolean))];
                ...
                >
                  ...
                  <div className="mt-8">
                    ...
                    <div className="flex justify-center mb-6">
                      <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-white/20 to-white/10 border border-white/30 flex items-center justify-center text-4xl group-hover:scale-110 transition-transform duration-300">
                        {club.logo}
                      </div>
                    </div>
                    ...
                    <div className="flex justify-center mb-4">
                      <div className="inline-flex items-center px-3 py-1 rounded-lg bg-gradient-to-r from-accent/20 to-orange-500/20 border border-accent/30">
                        <span className="text-accent font-medium text-sm">{club.country}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-white font-medium">{club.stadiumName || 'N/A'}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-white/60">Founded</span>
                        <span className="text-white font-medium">{club.foundedYear || 'N/A'}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-white/60">City</span>
                        <span className="text-white font-medium">{club.stadiumCity || 'N/A'}</span>
                      </div>
                    </div>
